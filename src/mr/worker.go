...
func Worker(mapf func(string, string) []KeyValue,
	...
	// into for loop, apply task from coordinator
	...
	for {
		...
		if reply.TaskType == MAP {
			...
			// transport data to MAP method and get temp result
			kva := mapf(reply.MapInputFile, string(content))
			// use hash result of Key to split.
			hashedKva := make(map[int][]KeyValue)
			for _, kv := range kva {
				hashed := ihash(kv.Key) % reply.ReduceNum
				hashedKva[hashed] = append(hashedKva[hashed], kv)
			}
			// write temp result into file
			for i := 0; i < reply.ReduceNum; i++ {
				ofile, _ := os.Create(tmpMapOutputFile(id, reply.TaskIndex, i))
			...
			for mi := 0; mi < reply.MapNum; mi++ {
				inputFile := finalMapOutputFile(mi, reply.TaskIndex)
			...
			ofile, _ := os.Create(tmpReduceOutputFile(id, reply.TaskIndex))
			i := 0
			for i < len(kva) {
				j := i + 1
